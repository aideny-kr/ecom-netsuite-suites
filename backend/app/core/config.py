...
class Settings(BaseSettings):
    ...
    # On by default (decided 2026-09-24). Per tenant, services/typesafe/access.py picks
    # the key (the tenant's own `typesafe` connection, else this platform key) and the
    # mode (the tenant's choice, live by default). No key anywhere = Jev off, and the
    # existing model path runs unchanged.
    ...
    BIGQUERY_ALLOWED_DATASETS: str = (
        ""  # comma-separated dataset allowlist for metric BigQuery exec; empty = no restriction
    )
