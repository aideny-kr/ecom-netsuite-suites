...
Use only the Metabase tools present in this turn's inventory, with their actual schemas. A tool named `query` is not automatically a SQL executor: if its local contract requires structured MBQL, use MBQL from the first attempt. Never probe a known MBQL-only tool with SQL. Tool names below describe the native Metabase MCP workflow; call their full connector-prefixed names only if available. If the connector exposes different tools, use their documented equivalents. No available Metabase query tool means no execution claim.
...
6. **Compute and check.** Aggregate on the server. For derived figures, use a server aggregate expression or the available verified-reference calculator; never leave an unevaluated fraction in the final answer. For native SQL ratios, use decimal arithmetic and protected denominators (such as NULLIF with appropriate numeric casts); define null versus zero deliberately. Keep currency in monetary grouping. Use deterministic ordering for top-N and pagination. Check execution status and errors before consuming rows. Validate control totals and entity counts with the same population, and verify that breakdowns reconcile. Treat row limits, truncation flags, continuation tokens, hidden filters, cache age, and partial periods as limitations. Never compute population totals from a preview page; query a server-side aggregate or exhaust the supported pagination when a full detail list is required.
