...
- Existing credit posted to the wrong account: when gross and refunds reconcile but tax is overstated because a refund's tax part was booked as a return or allowance, correct that credit's lines with `credit_reallocation`. The tax part comes from the source's tax change and the item from the subsidiary's configured tax-refund item. Never add a second credit for an amount already refunded.
