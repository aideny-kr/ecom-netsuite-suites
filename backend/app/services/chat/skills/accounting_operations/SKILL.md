...
Choose the document from the economic event and original agreement, not current source equality alone. A later return or concession can correctly leave the original invoice and sales order unchanged. If an existing credit already represents that event but its allocation is wrong, evaluate correcting that credit only; an invoice change could double-count the adjustment. A sales-order amendment requires independent evidence that the order itself is wrong and that changing it is appropriate for its lifecycle. Honor an explicit case resolution scope and its protected records.
