...
# Models on which output_config.effort="xhigh" is valid: Sonnet 5 / Opus 4.7+ / Fable.
# Other adaptive models (Sonnet 4.6, Opus 4.6) support "max" but NOT "xhigh" — sending
# "xhigh" to them 400s — so our internal "xhigh" maps to "max" there (the next step
# above "high"). Per Anthropic's effort docs (effort.md).
_XHIGH_EFFORT_MARKERS = ("sonnet-5", "opus-4-7", "opus-4-8", "fable", "mythos")


def anthropic_effort(level: str | None, model: str | None = None) -> str | None:
    ...
    if level == "xhigh":
        m = (model or "").lower()
        return "xhigh" if any(k in m for k in _XHIGH_EFFORT_MARKERS) else "max"
    return None


# Models on LEGACY extended thinking (thinking={type:enabled,budget_tokens}): the
# 4.5 / 4.0 / 4.1 line AND Haiku (Haiku supports extended thinking but NOT the effort
# param — adaptive+effort would error). Everything newer (Sonnet 5, Sonnet 4.6,
# Opus 4.6+, Fable) uses adaptive thinking + effort.
...
def thinking_mode(model: str | None) -> str:
    ...
    Defaults to 'adaptive' for unknown models — Anthropic's forward direction
    (budget_tokens is deprecated); the model is already VALID_MODELS-gated upstream.
    """
    m = (model or "").lower()
