...
def assemble_spec(title: str, sections: list[dict], resolver: Resolver) -> dict:
    ...
    for s, resolved in resolved_pairs:
        ...
        # statement table. Pop the internal hand-off key so it never persists into the
        # frozen spec_json. Gated to TABLE sections: only the resolver's own
        # freshly-built table dicts can carry the key legitimately (defense in depth on
        # top of the sanitized passthrough above — never honor it off any section shape
        # the model's dict could reach).
        callouts = None
