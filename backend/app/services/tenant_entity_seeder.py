...
async def seed_entity_mappings(
    ...
    result = await db.execute(
        text(
            "SELECT id, entity_type, script_id FROM tenant_entity_mapping "
            "WHERE tenant_id = :tid"
        ),
        {"tid": tenant_id},
    )
    stale_ids = [
        ...
        if (row.entity_type, row.script_id) not in current_keys
    ]
    if stale_ids:
        await db.execute(
            delete(TenantEntityMapping).where(TenantEntityMapping.id.in_(stale_ids))
        )
        logger.info(
