"""Celery task for executing workspace runs (SDF validate, Jest unit tests)."""
...
def workspace_run_task(self, tenant_id: str, run_id: str, **kwargs):
    ...
    try:
        return loop.run_until_complete(_execute(tenant_id, run_id))
    finally:
        loop.close()


async def _execute(tenant_id: str, run_id: str) -> dict:
    """Async inner: open session, set RLS, execute run."""
    async with async_session_factory() as session:
        await set_tenant_context(session, tenant_id)
        run = await runner_service.execute_run(session, uuid.UUID(run_id), uuid.UUID(tenant_id))
