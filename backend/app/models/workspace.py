...
class WorkspaceFile(Base, UUIDPrimaryKeyMixin, TimestampMixin):
    ...
    lock_holder: Mapped["User | None"] = relationship("User", foreign_keys=[locked_by])
...
class WorkspaceDeployToken(Base, UUIDPrimaryKeyMixin):
    ...
    issued_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default="now()", nullable=False)
