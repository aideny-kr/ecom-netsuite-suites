...
class TestMemoryConceptsInUnifiedAgent:
    ...
    def test_render_tolerates_missing_keys(self):
        """A malformed concept dict (missing keys) must not KeyError; an all-empty
        concept is skipped, a partial one still renders."""
        from app.services.chat.agents.unified_agent import UnifiedAgent

        agent = UnifiedAgent(tenant_id=uuid.uuid4(), user_id=uuid.uuid4(), correlation_id="t")
        agent._context = {"memory_concepts": [{}, {"name": "Solo"}]}
        p = agent.system_prompt  # must not raise
        assert "<tenant_memory>" in p
        assert "Solo" in p
