...
# ---------------------------------------------------------------------------
# Source question: answered once with "NetSuite", as a user would
# ---------------------------------------------------------------------------

_PENDING_SOURCE = {"version": 1, "kind": "analytics", "sources": [], "excluded_sources": [], "pending_source": True}
_SOURCE_QUESTION = "Which data source should I use for this question: BigQuery, Metabase or NetSuite?"


def _source_question_result(*, input_tokens: int = 1_000, output_tokens: int = 50) -> AgentResult:
    result = _make_agent_result(
        data=_SOURCE_QUESTION, input_tokens=input_tokens, output_tokens=output_tokens, confidence=None
    )
    result.request_context = dict(_PENDING_SOURCE)
    return result


@pytest.fixture
def scripted_turns(patched_deps):
    """Each new agent instance answers with the next scripted result and records its call."""
    from app.services.benchmarks import agent_runner
    ...
    def _init(self, *a, **kw):
        original_init(self, *a, **kw)
        self._canned_result = script[len(calls)]
    ...
    with (
        patch.object(agent_runner.UnifiedAgent, "__init__", _init),
        patch.object(agent_runner.UnifiedAgent, "run_streaming", _run_streaming),
    ):
        yield script, calls
...
class TestSourceQuestion:
    async def test_the_source_question_is_answered_with_netsuite_as_a_second_turn(self, scripted_turns, db_mock):
        from app.services.benchmarks import agent_runner
        ...
        assert result.success is True
        assert result.answer_text == "Top class is Laptops."
    ...
    async def test_a_second_source_question_fails_the_case_instead_of_looping(self, scripted_turns, db_mock):
        from app.services.benchmarks import agent_runner

        script, calls = scripted_turns
        script += [_source_question_result(), _source_question_result()]
    ...
    async def test_an_answer_without_a_source_question_runs_one_turn(self, scripted_turns, db_mock):
        ...
        script, calls = scripted_turns
        script += [_make_agent_result(data="42 orders.", input_tokens=10, output_tokens=5)]

        result = await agent_runner.run_agent(tenant_id=_TENANT_ID, question="Orders?", db=db_mock)

        assert result.success is True
        assert result.source_question_answered is False
        assert len(calls) == 1
        assert calls[0].get("conversation_history") is None
