...
async def finding(db, user, order, *, source=None, status="matched", at=None):
    ...
    run = await state_service.create_run(
        db, user.tenant_id, config.id,
        RunCreate(evaluation_key=str(at or status), order_references=(order.order_number,)), actor=user,
    )
    row = TransactionFinding(
        tenant_id=user.tenant_id, run_id=run.id, order_reference=order.order_number,
        report_json={
            "source": {"record_id": order.source_id},
            "balance": {"status": status, "currency": order.currency,
...
async def test_order_table_exposes_scoped_balance_and_filters_before_count_and_export(client, db, admin_user):
    ...
    response = await client.get("/api/v1/tables/orders", headers=headers,
                                params={"reconciliation_status": "matched", "page_size": 1})
    ...
    export = await client.get("/api/v1/tables/orders/export/csv", headers=headers,
                              params={"reconciliation_status": "matched"})
...
async def test_unrelated_or_stale_evidence_never_marks_order_matched(client, db, admin_user, admin_user_b, mismatch):
    ...
    source, evidence = await finding(
        db, admin_user_b[0] if mismatch == "tenant" else user, order,
    ...
    await db.flush()
    response = await client.get("/api/v1/tables/orders", headers=headers, params={"reconciliation_status": "not_verified"})
...
async def test_invalid_reconciliation_filter_is_not_silently_ignored(client, admin_user):
    for table, status in [("orders", "whatever"), ("payments", "matched")]:
        response = await client.get(f"/api/v1/tables/{table}", headers=admin_user[1],
