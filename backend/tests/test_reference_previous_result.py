...
class TestReferencePreviousResult:
    ...
    async def test_returns_cached_data(self):
        ...
        with patch(
            "app.mcp.tools.result_reference_tool.get_latest_result", new_callable=AsyncMock, return_value=mock_result
        ):
    ...
    async def test_specific_message_id(self):
        ...
        with patch(
            "app.mcp.tools.result_reference_tool.get_result_by_message",
