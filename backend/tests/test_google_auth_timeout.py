...
async def test_verify_google_token_times_out_instead_of_hanging(monkeypatch):
    ...
    def stalled_verify(*args, **kwargs):
        # Simulate the cert-fetch / verify call hanging far longer than the timeout.
        time.sleep(3)
    ...
    monkeypatch.setattr(gas, "GOOGLE_VERIFY_TIMEOUT", 0.3, raising=False)
    ...
    assert elapsed < 1.5, f"verify_google_token hung {elapsed:.1f}s instead of timing out"
