...
// Ported from frontend/src/components/chat/data-frame-table.tsx for the desktop
// renderer (rich-pipe slice 1). The table render is preserved verbatim; the
// webapp's React-Query / branding hooks (useCreateSavedQuery, useExcelExport)
// are reduced to OPTIONAL props (onSaveQuery, onExport) per the reuse scout
// verdict that the card is 100% presentational. With neither prop supplied
// (the slice-1 case) those controls simply don't render — the table, sort,
// copy, and client-side CSV are unchanged.
...
interface DataFrameTableProps {
  data: DataTableData;
  queryText?: string;
  /** Optional export hook (csv handled client-side; xlsx delegated to host). */
  onExport?: (format: "xlsx") => void;
  /** Optional save-to-analytics hook; when absent the Save control is hidden. */
  onSaveQuery?: (payload: {
    name: string;
    query_text: string;
    result_data: { columns: string[]; rows: unknown[][]; row_count: number };
  }) => void;
}

type SortDirection = "asc" | "desc" | null;

export function DataFrameTable({ data, queryText, onExport, onSaveQuery }: DataFrameTableProps) {
    ...
    <div className="my-3 overflow-hidden rounded-xl border bg-card shadow-soft">
      ...
      <div className="flex items-center justify-between border-t px-4 py-2">
        ...
        {queryText && onSaveQuery && (
          <SaveQueryButton queryText={queryText} columns={columns} rows={rows} rowCount={row_count} onSave={onSaveQuery} />
        )}
      </div>
    </div>
  );
...
// ---------------------------------------------------------------------------
// Save Query button — React-Query hook reduced to an injected onSave callback.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Helpers — verbatim from the webapp card.
// ---------------------------------------------------------------------------
