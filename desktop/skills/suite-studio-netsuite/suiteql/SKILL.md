...
description: SuiteQL dialect rules — Oracle-based with NetSuite-specific behaviors. Use whenever calling `ns_runSuiteQL` against NetSuite to avoid 0-row results, 400 errors, and inflated aggregates.
platforms: [linux, macos, windows]
---

# NetSuite SuiteQL — dialect rules

> Lifted **verbatim** from the Hosted backend's `backend/app/services/chat/knowledge_profiles/netsuite.yaml` (CLAUDE.md Mistakes #3: copy verbatim, never paraphrase). The single source of truth for SuiteQL dialect lives in the YAML; this SKILL.md mirrors the `prompt_fragment` block 1:1.
>
> Last sync: 2026-05-25 (`/goal` #3 — wiring `ns_runSuiteQL` into the sidecar).
...
## "list my NetSuite subsidiaries" — the canonical smoke query

```sql
SELECT id, name FROM subsidiary ORDER BY name
```

This is the gate-7 smoke test in `desktop/runtime/sidecar.py`. `subsidiary` is a standard NetSuite table; in a OneWorld account it returns multiple rows; in a non-OneWorld account it returns the single root subsidiary. Deterministic by design — no tenant-specific assumptions about transaction data shape.
