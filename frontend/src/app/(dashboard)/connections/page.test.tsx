...
vi.mock("@/hooks/use-connections", () => ({
  useConnections: () => ({ isError: mocks.fail, refetch: vi.fn(), data: [{ id: "api", provider: "solidus", label: "Framework", status: "active" }, { id: "gone", provider: "api", label: "Removed API", status: "revoked" }, { id: "celigo", provider: "celigo", label: "Celigo duplicate", status: "active" }, { id: "jev", provider: "typesafe", label: "TypeSafe Jev row", status: "active" }] }),
...
it("shows API and MCP controls and removes each through its own endpoint", async () => {
  ...
  expect(screen.queryByText("TypeSafe Jev row")).not.toBeInTheDocument();
