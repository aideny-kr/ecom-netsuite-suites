  ...
  return (
    <TooltipProvider delayDuration={300}>
      <div className="flex h-[calc(100vh-4rem)] flex-col">
        ...
        {/* ═══ Main Layout ═══ */}
        <PanelGroup id="workspace-h" orientation="horizontal" className="flex-1 overflow-hidden">
          {/* ─── Left: File Explorer ─── */}
          <Panel
            id="file-tree"
            ...
            collapsible
            collapsedSize={3}
            onResize={(size) => setFileTreeCollapsed(size.asPercentage <= 3)}
          >
            {fileTreeCollapsed ? (
              /* Collapsed strip */
              <div className="flex h-full flex-col items-center bg-muted/20 pt-2 gap-2">
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
          ...
          {/* ─── Right: Editor + Bottom ─── */}
          <Panel id="right-panel" defaultSize={78}>
            <PanelGroup id="workspace-v" orientation="vertical">
              {/* ─── Editor Panel ─── */}
              ...
              <PanelResizeHandle
                ...
                style={{ flexBasis: "8px" }}
              >
                <div className="pointer-events-none absolute inset-x-0 top-1/2 -translate-y-1/2 h-px bg-border group-hover:bg-primary/50 transition-colors" />
              </PanelResizeHandle>

              {/* ─── Bottom Panel ─── */}
              <Panel id="bottom-panel" defaultSize={40} minSize={8} maxSize={70} collapsible collapsedSize={4}>
                {selectedWorkspaceId ? (
                  <div className="flex h-full flex-col overflow-hidden">
                    {/* Bottom tab bar */}
                  ...
                  <div className="flex h-full items-center justify-center">
                    <div className="text-center space-y-2 text-muted-foreground">
                      <Terminal className="h-6 w-6 mx-auto text-muted-foreground/30" />
                      <p className="text-[12px]">Select a workspace to access tools</p>
                    </div>
                  </div>
                )}
              </Panel>
