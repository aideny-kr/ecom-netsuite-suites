...
```
User → nginx (Let's Encrypt SSL) on GCP VM
    ├── frontend:3000 (Next.js Docker container)
    └── backend:8000 (FastAPI + Celery workers)
...
|-----------|---------|------------|
| Frontend | Docker container on GCP (nginx + Let's Encrypt) | Docker container on GCP (nginx + Let's Encrypt) |
...
- **Stop Celery beat and worker BEFORE pulling new images.** Never `--force-recreate` all containers at once — this kills workers mid-refresh, consuming single-use OAuth refresh tokens that cannot be reused.
- **Deploy order:** Stop beat → stop worker → pull images → start backend → health check → start worker → start beat. Sequential restart prevents token storms.
- **Health endpoint:** `/health/detailed` returns pool stats + SSE counter for verification.

### Pre-Deploy Schema Validation (MANDATORY)

**Run BEFORE building Docker images** to catch model/DB mismatches:

```bash
cd backend && .venv/bin/python scripts/validate_schema.py
```

This script compares every SQLAlchemy model's mapped columns against the actual database
columns via `information_schema`. It catches:
- Migrations that DROP/ADD columns without updating the model (the `source_pin` bug)
- Models that add columns without a migration
- Any mismatch that would cause `UndefinedColumnError` in production

**Errors = hard stop.** Do NOT proceed with the build if validation fails.
Warnings (unmapped DB columns) are informational only and safe to ignore.

**Why this exists:** Migration 067 dropped `chat_sessions.source_pin` but the SQLAlchemy
model still had it as a `mapped_column`. This caused `UndefinedColumnError` on every
query touching `chat_sessions`, taking down the entire chat feature on staging.

The full pre-deploy checklist:
1. `cd backend && .venv/bin/python scripts/validate_schema.py` — schema validation
2. `docker buildx build ...` — build images
3. Deploy via sequential restart
...
# 4 uvicorn workers (scalability upgrade)
# No dev entrypoint — migrations run in CI
USER appuser
CMD ["uvicorn", "app.main:app", "--host", "0.0.0.0", "--port", "8000", "--workers", "4"]
```

Backend uses uvicorn directly (not gunicorn). Worker count was increased from 2 to 4 in Scalability Sprint 0 (2026-03-23), with stress testing confirming 25 concurrent chats at 100% success rate.
...
- Remote (Supabase): `pool_size=20, max_overflow=30, pool_recycle=300` (upgraded in Scalability Sprint 0)
- Local (Docker): `pool_size=20, max_overflow=10, pool_recycle=-1`

Production pool was increased from 5/5 to 20/30 to support higher concurrency after stress testing to 25 concurrent chats. Prefer `DATABASE_URL_DIRECT` (bypasses PgBouncer) for migrations and direct queries.
...
DEFAULT_FLAGS = {
    ...
    "custom_branding": False, "custom_domain": False, "unified_agent_enabled": True,
}
```
...
The `unified_agent_enabled` flag routes chat to the UnifiedAgent system on a per-tenant basis.
...
# NetSuite OAuth
# NOTE: No global NETSUITE_OAUTH_CLIENT_ID — client IDs are per-connection,
# stored encrypted in the `connections` and `mcp_connectors` tables.
...
Both frontend and backend are served from the same GCP VM via nginx as reverse proxy,
with Let's Encrypt SSL certificates (auto-renewed via certbot):

```nginx
server {
    listen 443 ssl;
    server_name suitestudio.ai;
    ssl_certificate /etc/letsencrypt/live/suitestudio.ai/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/suitestudio.ai/privkey.pem;
    location / { proxy_pass http://localhost:3000; }  # Next.js frontend
}

server {
    listen 443 ssl;
    server_name api.suitestudio.ai;
    ssl_certificate /etc/letsencrypt/live/api.suitestudio.ai/fullchain.pem;
    ssl_certificate_key /etc/letsencrypt/live/api.suitestudio.ai/privkey.pem;
    location / { proxy_pass http://localhost:8000; }  # FastAPI backend
}
```
...
```
suitestudio.ai     → GCP VM IP (frontend via nginx)
api.suitestudio.ai → GCP VM IP (backend via nginx)
```
...
**Latest migration:** `068_revoke_recon_ops` (as of 2026-04-14)
