...
- **Multi-angle review (T2):** `Workflow({name: "code-review-multiangle", args: {target: "<PR# or branch>"}})`. 8 angles: 7 Claude + 1 **independent-model (codex) angle** (the `grill-me` adversary, run read-only inside the gate) so the review is not Claude-on-Claude (which shares blind spots — see `memory/feedback_independent_model_review_gate`). It **fails CLOSED** — read the result's `status` FIRST:
  ...
  - **CHECK `target` AND `base` BEFORE READING ANY FINDING.** If `target` is `null`, the gate reviewed *the session cwd's current branch*, not what you asked for — and in this repo the session usually sits in a different worktree than the branch under review, so the default is normally WRONG. This has already produced a full 59-agent run against an unrelated branch whose findings were reported as if they applied. A run with `target: null` is void: re-run pinned. Confirm `base` matches the real PR base too.
  - every `UNVERIFIED` finding (a verifier failed) is preserved at `major` and needs human review.
  - resolve every CONFIRMED + PLAUSIBLE-major finding (fix, or defer with written rationale) before merge — but verify each against the code yourself first: a CONFIRMED finding has been a false positive before. Findings are evidence, not verdicts.
  - **one clean round is weak evidence.** Observed major counts across rounds on a single PR: `0 → 2 → 3 → 1 → 0`. A clean round straight after a dirty one means the last fix was clean, not that the work is. Budget for 2+ rounds, and treat a first-round zero on a large diff as a reason to re-check `target`.
