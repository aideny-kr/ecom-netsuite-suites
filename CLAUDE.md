...
- **Write-Back Confirmation (PR #39)**: HITL write confirmation via `mutation_guard.py` (detects `ext__` mutation tools, record-type allowlist of 23 types, HMAC token binding per session) + `write_confirmation_service.py` (builds payloads, validates on approval). Flow: agent calls write tool → `base_agent.py` intercepts via `classify_mutation()` before execution → yields `confirmation_required` event → orchestrator persists as `structured_output` → frontend `WriteConfirmationCard` renders before/after diff → user approves/rejects → orchestrator validates HMAC + executes (or cancels) + audit logs. Five safety layers: prompt instructions, tool-level hard gate, HMAC token, record-type allowlist, audit trail. Non-streaming `run()` blocks mutations entirely. Pre-fetch for update diffs uses `ns_getRecord` with 5s timeout.
- **Connector-Aware Agent Enabledness (PR #39)**: `requires_connector` field in `AgentYAMLConfig` declares which connectors an agent needs (e.g., bi-agent requires `["bigquery"]`). `AgentRegistry.get_enabled_agents()` filters by `_get_active_connectors()` — agents whose required connectors aren't active are excluded from all routing tiers. Resolves URGENT follow-up 54(a).
...
54. **Capability-sync open follow-ups** — (a) ✅ RESOLVED in PR #39: `requires_connector` field added to `AgentYAMLConfig`, bi-agent declares `requires_connector: ["bigquery"]`, `AgentRegistry.get_enabled_agents()` filters by active connectors. (b) Still open: Tenants with custom DB-stored prompt templates (`system_prompt_templates`) won't have `{{TOOL_INVENTORY}}` placeholder and will get an empty inventory — audit and backfill or add defensive fallback.
...
- **Product**: AI-den v1.11 deployed to staging 2026-04-15. PR #39: HITL write confirmation (mutation guard, HMAC tokens, WriteConfirmationCard, 5-layer safety) + connector-aware agent enabledness (requires_connector in YAML, AgentRegistry filtering). Prior: #35-#37 (v1.9-v1.10).
- **Latest migration**: 068_revoke_recon_ops (no new migrations in #39)
- **Frontend tests**: Vitest + @testing-library/react (40 tests). Run: `cd frontend && npx vitest run`
- **Backend tests**: 1,000+ tests. Run: `cd backend && .venv/bin/python -m pytest`
