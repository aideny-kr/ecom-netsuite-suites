...
- ~~**Never** hardcode NetSuite field names in Python. Every required-field fact comes from `ns_getRecordTypeMetadata` at runtime. The only hardcoded domain constants permitted are the two invariants in Task 5.~~ **REVERSED 2026-08-25 — this constraint was unsatisfiable.** `ns_getRecordTypeMetadata` returns a JSON-Schema catalog carrying field NAMES only: no `required` array, and `nullable` is `false` on zero of 177 customer fields. `ns_getSuiteQLMetadata` returns the same shape, and SuiteQL `CustomField.ismandatory` describes custom fields only. While the constraint stood, every card was `unvalidated: True` with `editable_slots: []` — required-field validation was inert and the agent chose values like `subsidiary` by reasoning alone. Requirements are now curated in `backend/app/services/chat/required_field_registry.py`, one reviewed line per fact, each carrying provenance; see STATE.md → DECIDED for the full rationale and the entries deliberately excluded. Runtime metadata is still the source for field NAMES and remains authoritative there.
