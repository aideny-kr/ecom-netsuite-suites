...
### 3.7 Oracle-side constraints (customer bill of materials, terms, trademark)
Sources: Oracle's own SuiteCloud SDK concurrency cheat sheet and SDF guide (UPL-1.0, on GitHub), a mirror of the NetSuite Subscription Services Agreement (SSA), Oracle's trademark guidelines (oracle.com, verbatim), and the setup docs of hosted vendors that already connect to customers' AI Connector (Dust, Speakeasy). Oracle price lists are not public; every dollar figure is a partner estimate.

- **Concurrency is the hard ceiling on selling point (e).** One account-wide pool is shared by REST, RESTlets, SOAP and the AI Connector: Standard tier 5 concurrent requests, Premium 15, Enterprise/Ultimate 20; each SuiteCloud Plus licence adds 10 (Standard allows one → ceiling 15; Premium 3 → 45; Enterprise 6 → 80; Ultimate 12 → 140). Sandbox and developer accounts are fixed at 5. Exceeding the cap returns HTTP 429 `CONCURRENCY_LIMIT_EXCEEDED`, and exceeding the SuiteCloud Plus cap contractually forces a tier upgrade with no mid-term downgrade. The backfill engine must therefore be rate-aware and honest: "5–15 parallel lanes on a Standard account" — anything beyond is an Oracle procurement event the customer did not budget for.
- **Customer bill of materials for Suite Studio:** Core Suite subscription (any tier; ~USD 999/month estimate); **one licensed Employee** as the service identity with a custom non-Administrator role carrying REST Web Services + "Log in using OAuth 2.0 Access Tokens" (+ "MCP Server Connection" for MCP) — the "Web Services Only" role flag blocks RESTlets, so the identity is a normal licensed seat (~USD 99–199/user/month estimate); optional SuiteCloud Plus for backfill concurrency (price undisclosed); optional Sandbox (separate SKU; OAuth integration tokens are destroyed on every refresh, so the sandbox path needs re-onboarding automation); SuiteAnalytics Connect only if a query must exceed SuiteQL's 100,000-row cap. The AI Connector / MCP Standard Tools SuiteApp has no documented SKU or fee; HIPAA/BAA accounts cannot activate it.
- **Terms.** SSA Section 1 licenses the service "solely for the internal business operations of Customer" but lets the customer authorise contractors and agents as Users, and "Third Party Applications" expressly include integrations, connectors and implementation services — a per-customer deployment operated as the customer's agent with the customer's own integration record fits. Two clauses bound the design: 3.1.2(c) forbids "timesharing or service bureau use" (one company per deployment, no shared credentials or database — exactly the pivot's shape) and 3.1.2(b) forbids using the service "to build or support … products or services competitive to Oracle" — so market Suite Studio as an operations/reconciliation layer that **extends** NetSuite, never as a replacement for NetSuite's AI. Every hosted deployment gets its own integration record created by the customer's admin (Authorization Code + PKCE, AI Connector scope, redirect at the customer's subdomain, consent policy "Always Ask"); never a shared client id across customers. Hosted third parties (Dust, Speakeasy) already do this; no "trusted client" allow-list was found, but the verbatim AI Connector acknowledgment text was not reachable and must be captured by a human before launch.
- **Distribution.** SDN membership is not required to publish the RESTlet and custom MCP tools on GitHub: the SSA grants every customer the right to share customer-developed SuiteApps, and Oracle itself publishes SDF projects (including MCP custom tools) under UPL-1.0. SDN (~USD 3,000/year, secondary source) matters only for a SuiteApp Marketplace listing or the Built for NetSuite badge — never use that phrase without certification. Do not redistribute the SuiteCloud SDK jar in public images: Oracle's Free Use Terms allow redistribution only unmodified and fee-free, and a hosting fee could be read as an "additional fee" — install the CLI at container start or in a separately built private image.
- **Trademark — the unresolved risk.** Oracle's guideline allows tag lines ("for NetSuite", "works with NetSuite" in smaller type, different font/colour, never Oracle red) and forbids "Oracle trademarks or potentially confusing variations as all or part of your company, product or service names" and in domain names. "Suite Studio" contains no Oracle mark and "suite" is generic, and third parties (SuiteSync, OpenSuiteMCP) use Suite-prefix names — but CamelCase "SuiteStudio", the `suitestudio.ai` domain and "for NetSuite" positioning together sit inside the "confusing variation" pattern Oracle enforces. Actions (P-01): professional USPTO/EUIPO clearance search in classes 9 and 42 and file our own application before the domain, repo and case study are public; always write "Suite Studio AI" as two words; Oracle credit line and a "not affiliated with, endorsed by, or certified by Oracle or NetSuite" notice on every page; pre-register a fallback name and domain so a rename is a config change, not a relaunch.

### 3.8 Compliance pack before a second company's data is hosted
Mostly contract language and one-page statements; two items are product changes. Costs are first-year, USD.

| Obligation | Applies | What must exist | Cost / time |
|---|---|---|---|
| GDPR / UK GDPR Art. 28 | yes (processor) | Signed DPA with Art. 28(3)(a)–(h) terms, Annex I processing details, Annex II TOMs, Annex III sub-processor list; SCC Module 3 (or 2) + UK Addendum + Swiss rider; transfer risk assessment; 30-day sub-processor change notice | 0–3K (templates from the Anthropic and Google DPAs + one lawyer review), 1–2 weeks |
| Sub-processor register | yes | Public page: Google Cloud (region per customer), Anthropic PBC / Anthropic Ireland (or Google only when Claude runs via Vertex), Sentry, Cloudflare, email provider | 0, 1 day |
| Anthropic terms | yes | Anthropic's DPA is auto-incorporated in the Commercial Terms; request per-organisation zero data retention from sales for the hosted org; disclose that the Covered Models (Fable 5.1 / Fable 5 / Mythos) require 30-day retention and that flagged content may be kept up to 2 years — so hosted instances default to Opus 5 / Sonnet 5 under ZDR | 0, 1–3 weeks (sales) |
| EU / UK data residency | conditional | Per-customer region variable routes Claude through **Vertex AI `eu` multi-region** (first-party API is US/global only); Postgres/Redis in europe-west1/4; no Files API / Batches on Vertex | +10% on tokens; 1–2 weeks engineering |
| PCI DSS | out of scope, conditional | One-page scoping statement (stores brand/last4/expiry/fingerprint only; no PAN or SAD; not a PCI service provider) + a PAN regex scanner on ingestion (NetSuite memo/custom fields, Shopify notes, uploaded files), chat transcripts and logs; attach Google Cloud's PCI DSS 4.0.1 AOC | 0, 2–3 days (avoids an 8–50K pen test) |
| CCPA / CPRA | when the customer is a California "business" | The 11 CCR §7051(a) service-provider clauses in the same DPA | 0 |
| EAR / OFAC | notification no; screening yes | `EXPORT.md` (publicly available source, standard cryptography only, 15 CFR 742.15(b)(1)); terms + sign-up geo-block for embargoed regions | 0, 1 day |
| EU AI Act | limited risk only | **"You are chatting with an AI system" disclosure in the chat UI and scheduled-job emails — Article 50 applies since 2 August 2026**; internal memo that no Annex III use exists (no natural-person credit scoring, no employee evaluation) | 0, 1 day |
| SOC 2 | deal gate | Type 1 then Type 2: platform 7–14K/year (Secureframe / Sprinto / Vanta Essentials) + boutique auditor 5–15K (Type 1) / 7–30K (Type 2); expect compensating controls for segregation of duties on a one-person team | 15–35K first year; Type 1 in 6–12 weeks, Type 2 in 5–9 months |
...
- Channels for the first five (60–90 days, ~USD 5–8K cash): (1) **CPA / fractional-CFO NetSuite practices** — 23 of 25 Platinum NetSuite partners are accounting firms and post-go-live support at ~USD 300/hour is their profit centre, so offer a 10–15% referral on the setup package and let the firm keep the support relationship; enrol in SuiteAccountants (free, 10% referral, no implementation commitment); target three signed referral letters by day 30. (2) **The Stripe gap** — Stripe's own NetSuite connector already does daily payout-to-deposit matching for Stripe-only merchants (price unpublished, partner-implemented, one deposit account per currency, fails on any unsynced payment); position the beachhead as everything it will not reconcile: Shopify Payments, PayPal, Amazon, multi-processor payouts, and the exception handling. (3) **Community as SEO** — three answers a week in the NetSuite Professionals Slack `#ai-netsuite` channel (its archive is Google-indexed) and r/Netsuite, no product links until asked. (4) **SuiteWorld 2026 (Oct 25–28, Las Vegas)** as an attendee, not exhibitor; partner happy hours; one free "Shopify + NetSuite finance ops" dinner in SF or NYC the fortnight before (Stacksync precedent). Kill criterion: zero qualified controller intros from the CPA track by day 60 → shift to a flat paid referral (~USD 4K per closed implementation) and a Celigo/A2X co-marketing ask. Well-funded direct competitors (Numeric, USD 89M raised; FloQast; Aprio's in-house agent) are logo- and content-led — do not try to outspend them.
...
| Oracle trademark objection to the name/domain, or SSA 3.1.2(b) "competitive" reading | medium | Clearance search + filing before publication; two-word name, non-affiliation notice, "extends NetSuite" positioning; fallback name pre-registered |
...
1. ~~Oracle terms and customer bill of materials~~ — answered in §3.7 (P-01, I-05, V-03). Still to do by a human: the verbatim AI Connector acknowledgment text, a professional trademark clearance for "Suite Studio", and Oracle's undisclosed prices for SuiteCloud Plus / tiers / sandbox from a partner quote.
2. ~~Compliance paperwork~~ — answered in §3.8 (F-06). Legal texts were read through mirrors; re-verify wording against the primary sources before a contract is signed.
3. ~~Channels for the first five customers~~ — answered in §6 (G-01, G-04). Member counts, SuiteWorld exhibitor tiers and referral rates were unverifiable through the research proxy.
4. Does the full stack fit `e2-standard-2` at Framework volume (400K+ payout lines)? Measure before promising RPO/RTO. (P-04)
5. Measured Framework token volume (Anthropic Usage API) — the USD 40/110/400 LLM figures are modeled. (P-04)
6. Framework's willingness to be a named public reference — every content and referral track depends on it. (P-03)
